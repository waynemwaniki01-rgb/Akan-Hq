...
import { L as require_jsx_runtime, R as require_react, _ as useRouter, f as createRouter, g as createRootRoute, h as createFileRoute, l as Scripts, m as lazyRouteComponent, p as Outlet, u as HeadContent } from "../_libs/@tanstack/react-router+[...].mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/router-Bu3nZ6EP.js
...
var styles_default = "/assets/styles-Dl6AuMnW.css";
var APP_NAME = "AKAN HQ";
var Route$5 = createRootRoute({
	head: () => ({
		meta: [
			...
			{
				name: "description",
				content: "AKAN HQ — football intelligence, tactical control and player performance. Squad cards, tactical board, simulation and a Card Designer."
			}
		],
		links: [
			{
...
var $$splitComponentImporter$1 = () => import("./routes-B-8R2xJp.mjs");
var Route$4 = createFileRoute("/")({ component: lazyRouteComponent($$splitComponentImporter$1, "component") });
var $$splitComponentImporter = () => import("./ask-coach-tY4poozZ.mjs");
var Route$3 = createFileRoute("/ask-coach")({ component: lazyRouteComponent($$splitComponentImporter, "component") });
function ensureEnvLoaded$1() {
	import_main.default.config({ path: "scripts/.env" });
}
function getFromAddress$1() {
	return `Aga Khan Football Academy <${process.env.GMAIL_USER}>`;
}
function getTransporter() {
	ensureEnvLoaded$1();
	const user = process.env.GMAIL_USER;
	const pass = process.env.GMAIL_APP_PASSWORD;
	if (!user || !pass) throw new Error("GMAIL_USER / GMAIL_APP_PASSWORD are not set — check scripts/.env");
...
* renders everywhere.
*/
function avatarBadge(name, size, ringColor, glow) {
	const inner = `<div style="width:${size}px;height:${size}px;border-radius:50%;background:linear-gradient(160deg,#1c2e26,#0c1411);display:flex;align-items:center;justify-content:center;">
       <span style="font-family:'Segoe UI',Arial,sans-serif;font-size:${Math.round(size * .34)}px;font-weight:800;color:#f0d79a;letter-spacing:1px;">${initialsOf(name)}</span>
     </div>`;
...
function coachAttributionBlock(coachName, label, accent) {
...
function calledUpEmail(r, callUpName, coachName, customNote) {
	const accent = "#d4b66a";
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
      ...
      <tr>
        <td style="padding:20px 28px 0;text-align:left;">
          ...
          <p style="font-family:'Segoe UI',Arial,sans-serif;color:#9fb0a8;font-size:13.5px;line-height:1.7;margin:22px 0 8px;">
            ${customNote || "Make sure to check with your coach for kickoff time, kit requirements, and where to meet. Train hard and represent the academy with pride."}
          </p>
        </td>
      </tr>
      <tr>
...
function notSelectedEmail(r, callUpName, coachName, customNote) {
	const accent = "#7d8f88";
	const position = safeText(r.position, "SQUAD");
	const playerName = safeText(r.name, "Player");
	return emailShell({
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
      ...
      <tr>
        <td style="padding:20px 28px 0;text-align:left;">
          ...
          <p style="font-family:'Segoe UI',Arial,sans-serif;color:#9fb0a8;font-size:13.5px;line-height:1.7;margin:22px 0 8px;">
            ${customNote || "Keep training hard — selection decisions are made week to week, and consistent effort at training is always noticed."}
          </p>
        </td>
      </tr>
      <tr>
...
async function sendOne(r, isCalled, callUpName, coachName, customNote) {
	if (!r.email) return {
		playerId: r.playerId,
		email: "",
		success: false,
	...
	try {
		...
		const from = getFromAddress$1();
		const info = await transporter.sendMail({
			from,
			to: r.email,
			subject: isCalled ? `⚽ You've been called up — ${callUpName}` : `Squad update — ${callUpName}`,
			html: isCalled ? calledUpEmail(r, callUpName, coachName, customNote) : notSelectedEmail(r, callUpName, coachName, customNote)
		});
...
var Route$2 = createFileRoute("/send-callup")({ server: { handlers: { POST: async ({ request }) => {
	ensureEnvLoaded$1();
	if (!process.env.GMAIL_USER || !process.env.GMAIL_APP_PASSWORD) {
		console.error("[send-callup] GMAIL_USER / GMAIL_APP_PASSWORD missing at request time — check scripts/.env");
		return new Response(JSON.stringify({ error: "Email service is not configured (Gmail credentials missing). Check scripts/.env." }), {
			status: 500,
	...
	if (!callUpName || !Array.isArray(called) || !Array.isArray(notSelected)) return new Response(JSON.stringify({ error: "Missing required fields" }), {
		status: 400,
		headers: { "Content-Type": "application/json" }
	});
	try {
		const results = await Promise.all([...called.map((r) => sendOne(r, true, callUpName, coachName, calledNote ?? null)), ...notSelected.map((r) => sendOne(r, false, callUpName, coachName, notSelectedNote ?? null))]);
		...
		return new Response(JSON.stringify({ error: err instanceof Error ? err.message : "Unexpected server error" }), {
			status: 500,
			headers: { "Content-Type": "application/json" }
		});
	}
...
function getResendClient() {
	...
	if (!apiKey) throw new Error("RESEND_API_KEY is not set — check scripts/.env");
	return new Resend(apiKey);
}
function askCoachEmail(playerName, callUpName, message) {
	const accent = "#d4b66a";
	return `
  <div style="font-family:'Segoe UI',Arial,sans-serif;background-color:#070c0a;padding:36px 16px;margin:0;">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:500px;margin:0 auto;">
          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
            <tr>
              <td style="padding:26px 28px;">
                ...
                ${message.split("\n").map((line) => `<p style="margin:0 0 10px;font-family:'Segoe UI',Arial,sans-serif;font-size:15px;line-height:1.6;color:#e4ece5;">${line.replace(/</g, "&lt;").replace(/>/g, "&gt;")}</p>`).join("")}
...
var Route$1 = createFileRoute("/api/ask-coach")({ server: { handlers: { POST: async ({ request }) => {
	ensureEnvLoaded();
	if (!process.env.RESEND_API_KEY) return new Response(JSON.stringify({ error: "Email service is not configured. Check scripts/.env." }), {
		status: 500,
		headers: { "Content-Type": "application/json" }
	});
	let body;
	...
	if (!coachEmail?.trim() || !playerName?.trim() || !message?.trim()) return new Response(JSON.stringify({ error: "Missing required fields" }), {
		status: 400,
		headers: { "Content-Type": "application/json" }
	});
	try {
		const { error } = await getResendClient().emails.send({
			from: getFromAddress(),
			to: coachEmail.trim(),
			replyTo: playerEmail?.trim() || void 0,
...
var rootRouteChildren = {
	...
	AskCoachRoute: Route$3.update({
		id: "/ask-coach",
		path: "/ask-coach",
		getParentRoute: () => Route$5
	}),
	SendCallupRoute: Route$2.update({
		id: "/send-callup",
		path: "/send-callup",
		getParentRoute: () => Route$5
	}),
	ApiAskCoachRoute: Route$1.update({
		id: "/api/ask-coach",
		path: "/api/ask-coach",
		getParentRoute: () => Route$5
	}),
...
//#endregion
export { getBaseURL as a, PACKAGE_VERSION as i, hasGateSessionMarker as n, GENERIC_OAUTH_ERROR_CODES as r, router_exports as t };
